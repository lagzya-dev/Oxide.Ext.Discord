# PlayerNameFormatter class

Formatter for player names

```csharp
public class PlayerNameFormatter
```

## Public Members

| name | description |
| --- | --- |
| static readonly [All](#All) | Include all name options in the player name |
| static readonly [ClanName](#ClanName) | Include clan name in the player name |
| static readonly [Default](#Default) | Default Player Name Formatter |
| static readonly [PlayerId](#PlayerId) | Include Player Id in the player name |
| static [Create](#Create)(…) | Create a new Player Name formatter with the given Custom Name Function |
| [Format](#Format)(…) | Formats the player name |

## See Also

* namespace [Oxide.Ext.Discord.Builders.Interactions.AutoComplete](./AutoCompleteNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
* [PlayerNameFormatter.cs](https://github.com/dassjosh/Oxide.Ext.Discord/blob/develop/Oxide.Ext.Discord/Builders/Interactions/AutoComplete/PlayerNameFormatter.cs)
   
   
# Create method

Create a new Player Name formatter with the given Custom Name Function

```csharp
public static PlayerNameFormatter Create(Func<IPlayer, string> customNameFunc)
```

| parameter | description |
| --- | --- |
| customNameFunc | Function to use for formatting the name |

## Return Value

A new [`PlayerNameFormatter`](./PlayerNameFormatter.md)

## See Also

* class [PlayerNameFormatter](./PlayerNameFormatter.md)
* namespace [Oxide.Ext.Discord.Builders.Interactions.AutoComplete](./AutoCompleteNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
   
   
# Format method

Formats the player name

```csharp
public string Format(IPlayer player)
```

| parameter | description |
| --- | --- |
| player | Player to have their name formatted |

## Return Value

Formatted player name

## See Also

* class [PlayerNameFormatter](./PlayerNameFormatter.md)
* namespace [Oxide.Ext.Discord.Builders.Interactions.AutoComplete](./AutoCompleteNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
   
   
# Default field

Default Player Name Formatter

```csharp
public static readonly PlayerNameFormatter Default;
```

## See Also

* class [PlayerNameFormatter](./PlayerNameFormatter.md)
* namespace [Oxide.Ext.Discord.Builders.Interactions.AutoComplete](./AutoCompleteNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
   
   
# ClanName field

Include clan name in the player name

```csharp
public static readonly PlayerNameFormatter ClanName;
```

## See Also

* class [PlayerNameFormatter](./PlayerNameFormatter.md)
* namespace [Oxide.Ext.Discord.Builders.Interactions.AutoComplete](./AutoCompleteNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
   
   
# PlayerId field

Include Player Id in the player name

```csharp
public static readonly PlayerNameFormatter PlayerId;
```

## See Also

* class [PlayerNameFormatter](./PlayerNameFormatter.md)
* namespace [Oxide.Ext.Discord.Builders.Interactions.AutoComplete](./AutoCompleteNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
   
   
# All field

Include all name options in the player name

```csharp
public static readonly PlayerNameFormatter All;
```

## See Also

* class [PlayerNameFormatter](./PlayerNameFormatter.md)
* namespace [Oxide.Ext.Discord.Builders.Interactions.AutoComplete](./AutoCompleteNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)

<!-- DO NOT EDIT: generated by xmldocmd for Oxide.Ext.Discord.dll -->
