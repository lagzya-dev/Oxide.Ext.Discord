# InteractionDataResolved class

Represents [Application Command Interaction Data Option](https://discord.com/developers/docs/interactions/receiving-and-responding#interaction-object-resolved-data-structure)

```csharp
public class InteractionDataResolved
```

## Public Members

| name | description |
| --- | --- |
| [InteractionDataResolved](#InteractionDataResolved)() | The default constructor. |
| [Attachments](#Attachments) { get; set; } | The ids and attachment objects |
| [Channels](#Channels) { get; set; } | The IDs and partial Channel objects |
| [Members](#Members) { get; set; } | The IDs and partial Member objects |
| [Messages](#Messages) { get; set; } | The ids and partial Message objects |
| [Roles](#Roles) { get; set; } | The IDs and Role objects |
| [Users](#Users) { get; set; } | The IDs and User objects |

## See Also

* namespace [Oxide.Ext.Discord.Entities.Interactions](./InteractionsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
* [InteractionDataResolved.cs](https://github.com/dassjosh/Oxide.Ext.Discord/blob/develop/Oxide.Ext.Discord/Entities/Interactions/InteractionDataResolved.cs)
   
   
# InteractionDataResolved constructor

The default constructor.

```csharp
public InteractionDataResolved()
```

## See Also

* class [InteractionDataResolved](./InteractionDataResolved.md)
* namespace [Oxide.Ext.Discord.Entities.Interactions](./InteractionsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Users property

The IDs and User objects

```csharp
public Hash<Snowflake, DiscordUser> Users { get; set; }
```

## See Also

* struct [Snowflake](../Snowflake.md)
* class [DiscordUser](../Users/DiscordUser.md)
* class [InteractionDataResolved](./InteractionDataResolved.md)
* namespace [Oxide.Ext.Discord.Entities.Interactions](./InteractionsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Members property

The IDs and partial Member objects

```csharp
public Hash<Snowflake, GuildMember> Members { get; set; }
```

## See Also

* struct [Snowflake](../Snowflake.md)
* class [GuildMember](../Guilds/GuildMember.md)
* class [InteractionDataResolved](./InteractionDataResolved.md)
* namespace [Oxide.Ext.Discord.Entities.Interactions](./InteractionsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Roles property

The IDs and Role objects

```csharp
public Hash<Snowflake, DiscordRole> Roles { get; set; }
```

## See Also

* struct [Snowflake](../Snowflake.md)
* class [DiscordRole](../Permissions/DiscordRole.md)
* class [InteractionDataResolved](./InteractionDataResolved.md)
* namespace [Oxide.Ext.Discord.Entities.Interactions](./InteractionsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Channels property

The IDs and partial Channel objects

```csharp
public Hash<Snowflake, DiscordChannel> Channels { get; set; }
```

## See Also

* struct [Snowflake](../Snowflake.md)
* class [DiscordChannel](../Channels/DiscordChannel.md)
* class [InteractionDataResolved](./InteractionDataResolved.md)
* namespace [Oxide.Ext.Discord.Entities.Interactions](./InteractionsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Messages property

The ids and partial Message objects

```csharp
public Hash<Snowflake, DiscordMessage> Messages { get; set; }
```

## See Also

* struct [Snowflake](../Snowflake.md)
* class [DiscordMessage](../Messages/DiscordMessage.md)
* class [InteractionDataResolved](./InteractionDataResolved.md)
* namespace [Oxide.Ext.Discord.Entities.Interactions](./InteractionsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Attachments property

The ids and attachment objects

```csharp
public Hash<Snowflake, MessageAttachment> Attachments { get; set; }
```

## See Also

* struct [Snowflake](../Snowflake.md)
* class [MessageAttachment](../Messages/MessageAttachment.md)
* class [InteractionDataResolved](./InteractionDataResolved.md)
* namespace [Oxide.Ext.Discord.Entities.Interactions](./InteractionsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)

<!-- DO NOT EDIT: generated by xmldocmd for Oxide.Ext.Discord.dll -->
