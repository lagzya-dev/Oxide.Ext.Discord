# GuildOnboarding class

Represents [Guild Onboarding Structure](https://discord.com/developers/docs/resources/guild#guild-onboarding-object-guild-onboarding-structure)

```csharp
public class GuildOnboarding
```

## Public Members

| name | description |
| --- | --- |
| [GuildOnboarding](#GuildOnboarding)() | The default constructor. |
| [DefaultChannelIds](#DefaultChannelIds) { get; set; } | Channel IDs that members get opted into automatically |
| [Enabled](#Enabled) { get; set; } | Whether onboarding is enabled in the guild |
| [GuildId](#GuildId) { get; set; } | ID of the guild this onboarding is part of |
| [Prompts](#Prompts) { get; set; } | Prompts shown during onboarding and in customize community |

## See Also

* namespace [Oxide.Ext.Discord.Entities.Guilds.Onboarding](./OnboardingNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
* [GuildOnboarding.cs](https://github.com/dassjosh/Oxide.Ext.Discord/blob/develop/Oxide.Ext.Discord/Entities/Guilds/Onboarding/GuildOnboarding.cs)
   
   
# GuildOnboarding constructor

The default constructor.

```csharp
public GuildOnboarding()
```

## See Also

* class [GuildOnboarding](./GuildOnboarding.md)
* namespace [Oxide.Ext.Discord.Entities.Guilds.Onboarding](./OnboardingNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
   
   
# GuildId property

ID of the guild this onboarding is part of

```csharp
public Snowflake GuildId { get; set; }
```

## See Also

* struct [Snowflake](../../Snowflake.md)
* class [GuildOnboarding](./GuildOnboarding.md)
* namespace [Oxide.Ext.Discord.Entities.Guilds.Onboarding](./OnboardingNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
   
   
# Prompts property

Prompts shown during onboarding and in customize community

```csharp
public List<OnboardingPrompt> Prompts { get; set; }
```

## See Also

* class [OnboardingPrompt](./OnboardingPrompt.md)
* class [GuildOnboarding](./GuildOnboarding.md)
* namespace [Oxide.Ext.Discord.Entities.Guilds.Onboarding](./OnboardingNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
   
   
# DefaultChannelIds property

Channel IDs that members get opted into automatically

```csharp
public List<OnboardingPrompt> DefaultChannelIds { get; set; }
```

## See Also

* class [OnboardingPrompt](./OnboardingPrompt.md)
* class [GuildOnboarding](./GuildOnboarding.md)
* namespace [Oxide.Ext.Discord.Entities.Guilds.Onboarding](./OnboardingNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)
   
   
# Enabled property

Whether onboarding is enabled in the guild

```csharp
public bool Enabled { get; set; }
```

## See Also

* class [GuildOnboarding](./GuildOnboarding.md)
* namespace [Oxide.Ext.Discord.Entities.Guilds.Onboarding](./OnboardingNamespace.md)
* assembly [Oxide.Ext.Discord](../../../../Oxide.Ext.Discord.md)

<!-- DO NOT EDIT: generated by xmldocmd for Oxide.Ext.Discord.dll -->
