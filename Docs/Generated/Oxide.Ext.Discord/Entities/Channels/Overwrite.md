# Overwrite class

Represents a [Overwrite Structure](https://discord.com/developers/docs/resources/channel#overwrite-object-overwrite-structure)

```csharp
public class Overwrite : ISnowflakeEntity
```

## Public Members

| name | description |
| --- | --- |
| [Overwrite](#Overwrite)() | The default constructor. |
| [Allow](#Allow) { get; set; } | Permissions allowed [`PermissionFlags`](../Permissions/PermissionFlags.md) |
| [Deny](#Deny) { get; set; } | Permissions denied [`PermissionFlags`](../Permissions/PermissionFlags.md) |
| [Id](#Id) { get; set; } | Role or user ID |
| [Type](#Type) { get; set; } | Permission Type [`PermissionType`](./PermissionType.md) |

## See Also

* interface [ISnowflakeEntity](../../Interfaces/ISnowflakeEntity.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
* [Overwrite.cs](https://github.com/dassjosh/Oxide.Ext.Discord/blob/develop/Oxide.Ext.Discord/Entities/Channels/Overwrite.cs)
   
   
# Overwrite constructor

The default constructor.

```csharp
public Overwrite()
```

## See Also

* class [Overwrite](./Overwrite.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Id property

Role or user ID

```csharp
public Snowflake Id { get; set; }
```

## See Also

* struct [Snowflake](../Snowflake.md)
* class [Overwrite](./Overwrite.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Type property

Permission Type [`PermissionType`](./PermissionType.md)

```csharp
public PermissionType Type { get; set; }
```

## See Also

* enum [PermissionType](./PermissionType.md)
* class [Overwrite](./Overwrite.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Allow property

Permissions allowed [`PermissionFlags`](../Permissions/PermissionFlags.md)

```csharp
public PermissionFlags? Allow { get; set; }
```

## See Also

* enum [PermissionFlags](../Permissions/PermissionFlags.md)
* class [Overwrite](./Overwrite.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Deny property

Permissions denied [`PermissionFlags`](../Permissions/PermissionFlags.md)

```csharp
public PermissionFlags? Deny { get; set; }
```

## See Also

* enum [PermissionFlags](../Permissions/PermissionFlags.md)
* class [Overwrite](./Overwrite.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)

<!-- DO NOT EDIT: generated by xmldocmd for Oxide.Ext.Discord.dll -->
