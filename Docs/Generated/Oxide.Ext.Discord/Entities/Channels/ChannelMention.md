# ChannelMention class

Represents a [Channel Mention Structure](https://discord.com/developers/docs/resources/channel#channel-mention-object-channel-mention-structure) in a message

```csharp
public class ChannelMention : ISnowflakeEntity
```

## Public Members

| name | description |
| --- | --- |
| [ChannelMention](#ChannelMention)() | The default constructor. |
| [GuildId](#GuildId) { get; set; } | ID of the guild containing the channel |
| [Id](#Id) { get; set; } | ID of the channel |
| [Name](#Name) { get; set; } | The name of the channel |
| [Type](#Type) { get; set; } | The type of channel [`ChannelType`](./ChannelType.md) |

## See Also

* interface [ISnowflakeEntity](../../Interfaces/ISnowflakeEntity.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
* [ChannelMention.cs](https://github.com/dassjosh/Oxide.Ext.Discord/blob/develop/Oxide.Ext.Discord/Entities/Channels/ChannelMention.cs)
   
   
# ChannelMention constructor

The default constructor.

```csharp
public ChannelMention()
```

## See Also

* class [ChannelMention](./ChannelMention.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Id property

ID of the channel

```csharp
public Snowflake Id { get; set; }
```

## See Also

* struct [Snowflake](../Snowflake.md)
* class [ChannelMention](./ChannelMention.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# GuildId property

ID of the guild containing the channel

```csharp
public Snowflake GuildId { get; set; }
```

## See Also

* struct [Snowflake](../Snowflake.md)
* class [ChannelMention](./ChannelMention.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Type property

The type of channel [`ChannelType`](./ChannelType.md)

```csharp
public ChannelType Type { get; set; }
```

## See Also

* enum [ChannelType](./ChannelType.md)
* class [ChannelMention](./ChannelMention.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Name property

The name of the channel

```csharp
public string Name { get; set; }
```

## See Also

* class [ChannelMention](./ChannelMention.md)
* namespace [Oxide.Ext.Discord.Entities.Channels](./ChannelsNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)

<!-- DO NOT EDIT: generated by xmldocmd for Oxide.Ext.Discord.dll -->
