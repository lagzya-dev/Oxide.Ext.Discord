# DiscordJsonWriter class

This is a pooled JSON writer that can write JSON to a stream

```csharp
public class DiscordJsonWriter : BasePoolable
```

## Public Members

| name | description |
| --- | --- |
| [DiscordJsonWriter](#DiscordJsonWriter)() | Constructor |
| static [Get](#Get)(…) | Returns a pooled [`DiscordJsonWriter`](./DiscordJsonWriter.md) |
| readonly [Stream](#Stream) | Stream that is written to |
| [Write](#Write)(…) | Writes the payload to the Stream |
| static [WriteAndCopy](#WriteAndCopy)(…) | Serializes the payload to the output stream |

## Protected Members

| name | description |
| --- | --- |
| override [EnterPool](#EnterPool)() |  |

## See Also

* class [BasePoolable](../../Pooling/BasePoolable.md)
* namespace [Oxide.Ext.Discord.Json.Serialization](./SerializationNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
* [DiscordJsonWriter.cs](https://github.com/dassjosh/Oxide.Ext.Discord/blob/develop/Oxide.Ext.Discord/Json/Serialization/DiscordJsonWriter.cs)
   
   
# Get method

Returns a pooled [`DiscordJsonWriter`](./DiscordJsonWriter.md)

```csharp
public static DiscordJsonWriter Get(DiscordPluginPool pluginPool)
```

## See Also

* class [DiscordPluginPool](../../Pooling/DiscordPluginPool.md)
* class [DiscordJsonWriter](./DiscordJsonWriter.md)
* namespace [Oxide.Ext.Discord.Json.Serialization](./SerializationNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# WriteAndCopy method

Serializes the payload to the output stream

```csharp
public static void WriteAndCopy(DiscordPluginPool pluginPool, JsonSerializer serializer, 
    object payload, Stream output)
```

| parameter | description |
| --- | --- |
| pluginPool | The [`DiscordPluginPool`](../../Pooling/DiscordPluginPool.md) to pool from |
| serializer | Serializer to use |
| payload | Payload to serialize |
| output | Output stream to write to |

## See Also

* class [DiscordPluginPool](../../Pooling/DiscordPluginPool.md)
* class [DiscordJsonWriter](./DiscordJsonWriter.md)
* namespace [Oxide.Ext.Discord.Json.Serialization](./SerializationNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Write method

Writes the payload to the Stream

```csharp
public void Write(JsonSerializer serializer, object payload)
```

| parameter | description |
| --- | --- |
| serializer | JsonSerializer to serialize with |
| payload | Payload to be serialized |

## See Also

* class [DiscordJsonWriter](./DiscordJsonWriter.md)
* namespace [Oxide.Ext.Discord.Json.Serialization](./SerializationNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# EnterPool method

```csharp
protected override void EnterPool()
```

## See Also

* class [DiscordJsonWriter](./DiscordJsonWriter.md)
* namespace [Oxide.Ext.Discord.Json.Serialization](./SerializationNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# DiscordJsonWriter constructor

Constructor

```csharp
public DiscordJsonWriter()
```

## See Also

* class [DiscordJsonWriter](./DiscordJsonWriter.md)
* namespace [Oxide.Ext.Discord.Json.Serialization](./SerializationNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)
   
   
# Stream field

Stream that is written to

```csharp
public readonly MemoryStream Stream;
```

## See Also

* class [DiscordJsonWriter](./DiscordJsonWriter.md)
* namespace [Oxide.Ext.Discord.Json.Serialization](./SerializationNamespace.md)
* assembly [Oxide.Ext.Discord](../../../Oxide.Ext.Discord.md)

<!-- DO NOT EDIT: generated by xmldocmd for Oxide.Ext.Discord.dll -->
